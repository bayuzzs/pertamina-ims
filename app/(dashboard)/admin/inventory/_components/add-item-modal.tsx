...
import type { CreateItemPayload, ItemStatus, ItemUnit } from "@/types/item";
import {
  Button,
  Input,
  Modal,
...
const addItemSchema = z.object({
  itemNo: z.string().trim().min(1, "Item number is required."),
...
const INITIAL_FORM: AddItemFormValues = {
  itemNo: "",
  name: "",
  description: "",
  vendor: "",
...
export default function AddItemModal({ accessToken }: AddItemModalProps) {
  ...
  const onSubmit = async (values: AddItemFormValues) => {
    ...
    try {
      ...
      setErrorMessage("Failed to add item. Please try again in a moment.");
      if (error instanceof AxiosError) {
        errorToast(
          error.response?.data?.message ||
            "Failed to add item. Please try again in a moment.",
        );
      }
    }
  };
            ...
            <form onSubmit={handleSubmit(onSubmit)}>
              <ModalHeader className="flex flex-col gap-1">
                ...
                <p className="text-default-500 text-xs font-normal">
                  Fill in the item details to add it to the inventory master
                  data.
                </p>
              </ModalHeader>
